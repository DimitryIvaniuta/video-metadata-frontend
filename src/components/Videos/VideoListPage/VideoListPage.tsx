...
import { VideoProvider, VideoSort, useGetConnectionVideosQuery } from '@/graphql/generated/graphql';
...
export const VideoListPage: React.FC = () => {
    ...
    const providers: (VideoProvider | 'ALL')[] = [
        'ALL',
        ...Object.values(VideoProvider).filter((v): v is VideoProvider => typeof v === 'string'),
    ];
    const [providerFilter, setProviderFilter] = useState<'ALL' | VideoProvider>('ALL');
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <select
                    ...
                    onChange={e => { setProviderFilter(e.target.value as any); setPage(1); }}
            ...
            <div className="overflow-x-auto bg-white shadow rounded-lg">
                <table className="min-w-full table-auto">
                    ...
                    <tr>
                        ...
                        >
                            Upload Date {sortBy === VideoSort.UploadDate && (sortDesc ? '↓' : '↑')}
                        </th>
                    ...
                    {!loading && !error && items.filter(v=>v!==null)
                        .map(video => (
                        <tr key={video.id} className="border-t hover:bg-gray-50">
                            ...
                            <td className="px-4 py-2">
                                {!video.durationMs ? 0 : Math.floor(video.durationMs / 60000)}:
                                {!video.durationMs ? '' : String(Math.floor((video.durationMs % 60000) / 1000)).padStart(2, '0')}
                            </td>
                            <td className="px-4 py-2">
                                {video.uploadDate && formatDistanceToNow(parseISO(video.uploadDate), { addSuffix: true })}
                            </td>
                        </tr>
                    ))}
                    </tbody>
